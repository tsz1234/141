...
def upload_csv_mapreduce_hadoop():
    # 查询数据
    query = "SELECT * FROM traveldemand"
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"traveldemand.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/traveldemand.csv'
    # 目标 HDFS 路径
...
def send_cmd():
    ...
    [
        ...
        "-mapper", f"\"python value_mapper.py {csv_index('traveldemand.csv','starttime')} {csv_index('traveldemand.csv','qixingshizhang')} 日  \"",
        "-reducer", "\"python value_reducer.py starttime\"",
        "-input", "hdfs://localhost:9000/input/traveldemand.csv",
        "-output", "hdfs://localhost:9000/output/traveldemand/valuestarttimeqixingshizhangdate"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('traveldemand.csv','starttime')} {csv_index('traveldemand.csv','qixingjuli')} 日  \"",
        "-reducer", "\"python value_reducer.py starttime\"",
        "-input", "hdfs://localhost:9000/input/traveldemand.csv",
        "-output", "hdfs://localhost:9000/output/traveldemand/valuestarttimeqixingjulidate"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('traveldemand.csv','starttime')} {csv_index('traveldemand.csv','hourlyrentalquantity')} 日  \"",
        "-reducer", "\"python value_reducer.py starttime\"",
        "-input", "hdfs://localhost:9000/input/traveldemand.csv",
        "-output", "hdfs://localhost:9000/output/traveldemand/valuestarttimehourlyrentalquantitydate"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('traveldemand.csv','starttime')} {csv_index('traveldemand.csv','traveldemand')} 日  \"",
        "-reducer", "\"python value_reducer.py starttime\"",
        "-input", "hdfs://localhost:9000/input/traveldemand.csv",
        "-output", "hdfs://localhost:9000/output/traveldemand/valuestarttimetraveldemanddate"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('traveldemand.csv','kaishizhandian')}\"",
        "-reducer", "\"python group_reducer.py kaishizhandian\"",
        "-input", "hdfs://localhost:9000/input/traveldemand.csv",
        "-output", "hdfs://localhost:9000/output/traveldemand/groupkaishizhandian"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('traveldemand.csv','jieshuzhandian')}\"",
        "-reducer", "\"python group_reducer.py jieshuzhandian\"",
        "-input", "hdfs://localhost:9000/input/traveldemand.csv",
        "-output", "hdfs://localhost:9000/output/traveldemand/groupjieshuzhandian"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('traveldemand.csv','dangritianqi')}\"",
        "-reducer", "\"python group_reducer.py dangritianqi\"",
        "-input", "hdfs://localhost:9000/input/traveldemand.csv",
        "-output", "hdfs://localhost:9000/output/traveldemand/groupdangritianqi"
    ],
    ]
